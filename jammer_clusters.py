...
SELECT  
  ...
  where distancia <= 1600 
  and EventId_origen != EventId_destino
  --and distancia != 0
  group by EventId_origen 
  ) a
  where cluster_size >= 2
  and EventId_origen not in ( select Eventid from [Mapping_Tools].[work].[clusters_Jammer])
  and EventId_origen not in ( select centro from [Mapping_Tools].[work].[clusters_Jammer])
...
cursor2.execute("""  select max(cast(cluster_id as int))
    FROM [Mapping_Tools].[work].[clusters_Jammer]""")
...
""")
    ...
    for i in row:
        ...
        cursor.execute("""
                       insert into [Mapping_Tools].[work].[clusters_Jammer]
        select """+str(count)+""" cluster_id, EventId_origen centro, EventId_destino id
  FROM [Mapping_Tools].[Staging].[Jammer_distancias]
  where distancia <= 1600
  and EventId_origen = '"""+str(i[0])+"""'
  and EventId_origen != EventId_destino
  and EventId_destino not in (select Eventid from [Mapping_Tools].[work].[clusters_Jammer])
  and EventId_destino not in (select centro from [Mapping_Tools].[work].[clusters_Jammer])
        """)
        cursor.commit()
